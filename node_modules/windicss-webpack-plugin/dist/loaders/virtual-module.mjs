...
import { j as MODULE_ID_VIRTUAL_TEST, h as debug, k as def } from '../shared/windicss-webpack-plugin.e34314d2.mjs';
import 'debug';
...
async function VirtualModule(source) {
  ...
  const generateCSS = async (layer2) => {
    try {
      ...
      service.virtualModules.set(def(layer2, "all"), css);
      callback(null, css);
    } catch (e) {
...
/* Error: ${error}*/`);
  ...
  if (service.dirty.has("all-modules")) {
    const contents = await Promise.all(
      [...await service.getFiles()].filter((id) => service.isDetectTarget(id)).map(async (id) => [await fs.promises.readFile(id, "utf-8"), id])
    );
